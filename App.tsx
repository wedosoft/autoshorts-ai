...
const VISUAL_STYLES = [
  { 
    ...
    label: '실사 (Realistic)', 
    icon: <Camera size={18} />, 
    prompt: 'Photorealistic, RAW photo, 8k UHD, film grain, Fujifilm XT4, cinematic lighting, realistic skin textures, shot on 35mm lens. ABSOLUTELY NO ILLUSTRATION, NO CARTOON, NO PAINTING LOOK.' 
  },
  { 
    id: 'anime', 
    label: '애니메이션 (Anime)', 
    icon: <Sparkles size={18} />, 
    prompt: 'High-quality modern Japanese anime style, Makoto Shinkai inspired, vibrant colors, clean cel-shaded lines, beautiful backgrounds, cinematic composition.' 
  },
  { 
    id: '3d_cartoon', 
    label: '3D 툰 (Pixar)', 
    icon: <Ghost size={18} />, 
    prompt: 'High-end 3D animation style, Pixar and Disney inspired, subsurface scattering, soft professional studio lighting, expressive character design, Octane Render.' 
  },
  { 
    id: 'cyberpunk', 
    label: '사이버펑크 (Neon)', 
    icon: <Wand2 size={18} />, 
    prompt: 'Cyberpunk 2077 aesthetic, volumetric neon lighting, rainy futuristic city, high contrast purple and cyan palette, cinematic sci-fi photography.' 
  },
  { 
    id: 'oil_painting', 
    label: '유화 (Oil Paint)', 
    icon: <Brush size={18} />, 
    prompt: 'Classical oil painting on canvas, heavy impasto brushstrokes, rich oily textures, dramatic chiaroscuro lighting, fine art museum quality.' 
  },
  { 
    id: 'sketch', 
    label: '스케치 (Sketch)', 
    icon: <PenTool size={18} />, 
    prompt: 'Hand-drawn charcoal and pencil sketch, artistic hatching and cross-hatching, graphite textures on textured paper, expressive strokes, black and white.' 
  },
];
  ...
  const handleGenerate = useCallback(async (forcedTopic?: string) => {
    ...
    try {
      ...
      for (let i = 0; i < sceneCount; i++) {
        ...
        // We pass both the global style and the scene visual prompt
        ...
        if (!isQuotaExhausted) {
          ...
          try {
            ...
            console.warn("Veo Quota Hit, switching to Cinematic Image mode.");
      ...
      console.error(err);
      setError(err.message || "생성 과정에서 오류가 발생했습니다.");
    ...
    <div className="min-h-screen flex flex-col items-center py-12 px-4 font-sans">
      <div className="text-center mb-16 space-y-4 animate-fade-in">
        ...
        <p className="text-cyan-100/70 text-lg md:text-xl max-w-lg mx-auto leading-relaxed word-keep-all drop-shadow-md">
          SaaS 최적화 하이브리드 엔진이 탑재되었습니다.<br/>
          사용자의 스타일 선택을 엄격히 준수하여 고퀄리티 에셋을 생성합니다.
        </p>
      </div>
        ...
        {(state !== AppState.IDLE && state !== AppState.COMPLETED && state !== AppState.ERROR) && (
            <div className="mt-20 text-center space-y-8 animate-pulse">
                ...
                <div className="space-y-4">
                    ...
                    <p className="text-cyan-200/40 text-sm max-w-sm mx-auto">선택하신 "{VISUAL_STYLES.find(s => s.id === selectedStyle)?.label}" 스타일을 엄격히 적용 중입니다.</p>
                </div>
            </div>
        )}
        ...
        {state === AppState.ERROR && (
            <div className="mt-8 p-10 bg-white/10 backdrop-blur-2xl border border-red-500/30 rounded-3xl max-w-md w-full text-center shadow-2xl">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <button onClick={resetApp} className="w-full px-6 py-4 bg-white/5 text-white border border-white/10 rounded-xl font-bold">새 프로젝트</button>
                </div>
            </div>
        )}
      </div>
